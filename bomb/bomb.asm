...
0000000000401062 <phase_5>: #9on567
  401062:	53                   	push   %rbx 
  401063:	48 83 ec 20          	sub    $0x20,%rsp # %rsp = %rsp - 32
  401067:	48 89 fb             	mov    %rdi,%rbx # %rbx = %rsi = input
  40106a:	64 48 8b 04 25 28 00 	mov    %fs:0x28,%rax # %rax = canary
  401071:	00 00 
  401073:	48 89 44 24 18       	mov    %rax,0x18(%rsp) # (%rsp + 24) = %rax = canary
  401078:	31 c0                	xor    %eax,%eax # %rax = %rax ^ %rax = 0
  40107a:	e8 9c 02 00 00       	callq  40131b <string_length> # get string length of input
  40107f:	83 f8 06             	cmp    $0x6,%eax # compare %rax : 6
  401082:	74 4e                	je     4010d2 <phase_5+0x70> # if %rax == 6, go on
  401084:	e8 b1 03 00 00       	callq  40143a <explode_bomb> # else, boom!
  401089:	eb 47                	jmp    4010d2 <phase_5+0x70> #
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx # loop, %rcx = (%rbx + %rax), the value at the i index of input, i = %rax, save in stack
  40108f:	88 0c 24             	mov    %cl,(%rsp) # (%rsp) = %cl
  401092:	48 8b 14 24          	mov    (%rsp),%rdx # %rdx = (%rsp) = %cl
  401096:	83 e2 0f             	and    $0xf,%edx # %rdx = %rdx & 0x1111
  ...
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax # compare %rax : 6
  4010ac:	75 dd                	jne    40108b <phase_5+0x29> # if not equal, into loop
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp) # else, (22 + %rsp) = 0, add the end character at end of the string
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi # %rsi = 0x40245e = "flyers"
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi # save the address of %rsp + 16 at %rdi
  4010bd:	e8 76 02 00 00       	callq  401338 <strings_not_equal>
  4010c2:	85 c0                	test   %eax,%eax # if equal, array[input[i]&0xf] == target[i], go on
  4010c4:	74 13                	je     4010d9 <phase_5+0x77>
  4010c6:	e8 6f 03 00 00       	callq  40143a <explode_bomb> # if not equal, boom!
  4010cb:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)
  4010d0:	eb 07                	jmp    4010d9 <phase_5+0x77>
  4010d2:	b8 00 00 00 00       	mov    $0x0,%eax # %rax = 0
  4010d7:	eb b2                	jmp    40108b <phase_5+0x29> # into loop
  4010d9:	48 8b 44 24 18       	mov    0x18(%rsp),%rax # %rax = (%rsp + 24)
  4010de:	64 48 33 04 25 28 00 	xor    %fs:0x28,%rax # %rax = %rax ^ canary
  4010e5:	00 00 
  4010e7:	74 05                	je     4010ee <phase_5+0x8c> # if equal, return
  4010e9:	e8 42 fa ff ff       	callq  400b30 <__stack_chk_fail@plt> # else, stack has been destructed
  4010ee:	48 83 c4 20          	add    $0x20,%rsp
  4010f2:	5b                   	pop    %rbx
  4010f3:	c3                   	retq   

00000000004010f4 <phase_6>: # 4 3 2 1 5 6
  ...
  401103:	48 89 e6             	mov    %rsp,%rsi # set the second argument of the func read six numbers
  401106:	e8 51 03 00 00       	callq  40145c <read_six_numbers> # call read_six_numbers
  40110b:	49 89 e6             	mov    %rsp,%r14 # %r14 = %rsp 
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d # the lower 32 bit of %r12 set to 0
  401114:	4c 89 ed             	mov    %r13,%rbp # %rbp = %r13
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax # %rax = (%r13) = a1
  40111b:	83 e8 01             	sub    $0x1,%eax # %rax -= 1
  40111e:	83 f8 05             	cmp    $0x5,%eax # compare %rax : 5
  401121:	76 05                	jbe    401128 <phase_6+0x34> # if below or equal, go on
  401123:	e8 12 03 00 00       	callq  40143a <explode_bomb> # else, boom!
  401128:	41 83 c4 01          	add    $0x1,%r12d # the lower 32 bit of %r12 add 1
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d # compare %r12d : 6
  401130:	74 21                	je     401153 <phase_6+0x5f> # if equal, go on
  ...
  401160:	89 ca                	mov    %ecx,%edx # %rdx = %rcx = 7, loop1, change argumenti into 7 - argumenti
  ...
  40116d:	75 f1                	jne    401160 <phase_6+0x6c> # if not equal, jump into loop1
  40116f:	be 00 00 00 00       	mov    $0x0,%esi # %rsi = 0
  401174:	eb 21                	jmp    401197 <phase_6+0xa3> # jump into
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx
  40117a:	83 c0 01             	add    $0x1,%eax
  40117d:	39 c8                	cmp    %ecx,%eax
  40117f:	75 f5                	jne    401176 <phase_6+0x82>
  401181:	eb 05                	jmp    401188 <phase_6+0x94>
  401183:	ba d0 32 60 00       	mov    $0x6032d0,%edx # %rdx = 0x6032d0, loop5
  401188:	48 89 54 74 20       	mov    %rdx,0x20(%rsp,%rsi,2) # (32 + %rsp + 2 * %rsi) = %rdx
  40118d:	48 83 c6 04          	add    $0x4,%rsi # %rsi += 4
  401191:	48 83 fe 18          	cmp    $0x18,%rsi # comapre %rsi : 24
  401195:	74 14                	je     4011ab <phase_6+0xb7> # if equal, jump into 
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx # %rcx = (%rsp + %rsi) 
  40119a:	83 f9 01             	cmp    $0x1,%ecx # compare %rcx : 1
  40119d:	7e e4                	jle    401183 <phase_6+0x8f> # if less or equal, jump into loop
  40119f:	b8 01 00 00 00       	mov    $0x1,%eax
  4011a4:	ba d0 32 60 00       	mov    $0x6032d0,%edx
  4011a9:	eb cb                	jmp    401176 <phase_6+0x82>
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx # %rbx = (%rsp + 32)
  4011b0:	48 8d 44 24 28       	lea    0x28(%rsp),%rax # %rax = the address of %rsp + 40
  ...
  4011c4:	48 83 c0 08          	add    $0x8,%rax # %rsx += 8
  4011c8:	48 39 f0             	cmp    %rsi,%rax # compare %rax : %rsi
  4011cb:	74 05                	je     4011d2 <phase_6+0xde> # if equal, go on 
  4011cd:	48 89 d1             	mov    %rdx,%rcx
  4011d0:	eb eb                	jmp    4011bd <phase_6+0xc9>
  4011d2:	48 c7 42 08 00 00 00 	movq   $0x0,0x8(%rdx) # (%rdx + 8) = 0
  4011d9:	00 
  4011da:	bd 05 00 00 00       	mov    $0x5,%ebp # %rbp = 5
  4011df:	48 8b 43 08          	mov    0x8(%rbx),%rax # %rax = (%rbx + 8), loop2
  4011e3:	8b 00                	mov    (%rax),%eax # %rax = (%rax)
  4011e5:	39 03                	cmp    %eax,(%rbx) # compare (%rbx) : %rax
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa> # if great or equal, go on
  ...
  4011f5:	75 e8                	jne    4011df <phase_6+0xeb> # if not equal, jump into loop2
...
000000000040131b <string_length>:
  40131b:	80 3f 00             	cmpb   $0x0,(%rdi) # check the string is 0 or not
  40131e:	74 12                	je     401332 <string_length+0x17> # if string is null, return length 0 
  401320:	48 89 fa             	mov    %rdi,%rdx # %rdx is the address of the string
  401323:	48 83 c2 01          	add    $0x1,%rdx # read the next char in the string
  401327:	89 d0                	mov    %edx,%eax # %rax is the address of the next string
  401329:	29 f8                	sub    %edi,%eax # %rax is the current string's length
  40132b:	80 3a 00             	cmpb   $0x0,(%rdx) # check the string is end or not
  40132e:	75 f3                	jne    401323 <string_length+0x8> # if the string is not an the end, continue loop
  401330:	f3 c3                	repz retq  #return string
...
0000000000401338 <strings_not_equal>:
  ...
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx
  40133c:	48 89 fb             	mov    %rdi,%rbx # the first argument saved in %rbx
  40133f:	48 89 f5             	mov    %rsi,%rbp # the second argument saved in %rbp
  401342:	e8 d4 ff ff ff       	callq  40131b <string_length> # call the function using the first argument
  401347:	41 89 c4             	mov    %eax,%r12d # save the return value in %r12d
  40134a:	48 89 ef             	mov    %rbp,%rdi # past the second argument as the only argument for below function
  40134d:	e8 c9 ff ff ff       	callq  40131b <string_length> # call the function
  401352:	ba 01 00 00 00       	mov    $0x1,%edx # set %rdx to 1, in case of as the return value
  401357:	41 39 c4             	cmp    %eax,%r12d # compare the return values of the two function
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63> # if not equal, %rax = 1, return 1  
  40135c:	0f b6 03             	movzbl (%rbx),%eax # saved the value at address %rbx into the %rax
  40135f:	84 c0                	test   %al,%al # the lower 8 bit of %rax has the first bit value of string, if it is 0x00, meaning is end character
  401361:	74 25                	je     401388 <strings_not_equal+0x50> # if equal, meaning the input string and the true answer are both null string
  401363:	3a 45 00             	cmp    0x0(%rbp),%al # compare the asnwer string's first element with our input string's first element
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a>  # if equal, jump to check the second element of the input string
  401368:	eb 25                	jmp    40138f <strings_not_equal+0x57> # not equal, return 1
  40136a:	3a 45 00             	cmp    0x0(%rbp),%al # while loop, compare the next element of both string
  40136d:	0f 1f 00             	nopl   (%rax)
  401370:	75 24                	jne    401396 <strings_not_equal+0x5e> # if not equal, return 1
  401372:	48 83 c3 01          	add    $0x1,%rbx # %rbx is the address of the second element of the input string
  401376:	48 83 c5 01          	add    $0x1,%rbp # %rbp is the address of the second element of the answer string
  40137a:	0f b6 03             	movzbl (%rbx),%eax # %rax is the input string move off the first element 
  40137d:	84 c0                	test   %al,%al #if hit the end of the input string, return 0, answer string equal input string
  40137f:	75 e9                	jne    40136a <strings_not_equal+0x32> # the input string not hit the end, while loop
